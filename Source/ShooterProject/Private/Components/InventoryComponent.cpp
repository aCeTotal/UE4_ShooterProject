...
FItemAddResult UInventoryComponent::TryAddMagazineFromClass(TSubclassOf<class UItem> ItemClass, const int32 Quanity, const int32 Ammo)
{
	UItem* Item = NewObject<UItem>(GetOwner(), ItemClass);
	Item->SetQuantity(Quanity);
	Item->SetAmmoAmount(Ammo);
	return TryAddMagazine_Internal(Item);
}
...
UItem* UInventoryComponent::AddMagazine(class UItem* Item)
{
	if (GetOwner() && GetOwner()->HasAuthority()) //IF Server
		{
		//Duplicates the object and creates a new one with the correct owner.
...
FItemAddResult UInventoryComponent::TryAddMagazine_Internal(class UItem* Item)
{
	if (GetOwner() && GetOwner()->HasAuthority())
	{
		//Amount to be added
		const int32 AddAmount = Item->GetQuantity();
		const int32 AddAmmoAmount = Item->GetAmmo();
